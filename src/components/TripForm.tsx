...
const TripForm: React.FC<TripFormProps> = ({ onSaveTrip }) => {
  ...
  const handleAddStop = () => {
    const newStop: Stop = {
      id: uuidv4(),
      destination: '',
      departureLocation: stops.length > 0 ? stops[stops.length - 1].destination : destination,
      activity: '',
      arrivalTime: '',
      departureTime: '',
  ...
  const vehicleOptions = [
    { value: 'van-passageiro-01', label: 'VAN PASSAGEIRO (TIPO 01)', icon: Bus },
    { value: 'servico-01', label: 'SERVIÇO (TIPO 01)', icon: Car },
    { value: 'picape-01', label: 'PICAPE (TIPO 01)', icon: Truck },
    { value: 'van-carga-01', label: 'VAN DE CARGA (TIPO 01)', icon: Bus },
    { value: 'van-passageiro-02', label: 'VAN PASSAGEIRO (TIPO 02)', icon: Bus },
    { value: 'picape-02', label: 'PICAPE (TIPO 02)', icon: Truck },
    { value: 'motocicleta-02', label: 'Motocicleta (Tipo 2)', icon: Car },
    { value: 'furgao-carga-01', label: 'FURGÃO DE CARGA (TIPO 01)', icon: Bus },
    { value: 'caminhao-bau', label: 'CAMINHÃO BAÚ 3/4', icon: Truck },
    { value: 'servico-02', label: 'SERVIÇO (TIPO 02)', icon: Car },
          ...
          <SelectTrigger className="w-full">
            <SelectValue>
              <div className="flex items-center space-x-2">
                {getVehicleIcon()}
                <span>{vehicleOptions.find(opt => opt.value === vehicleType)?.label || vehicleType}</span>
              </div>
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Local de onde saiu"
                    disabled
                  />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-2">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Qual atividade foi feita"
                  />
                </div>
